...
# Provision ADN topology in Tenant_Restricted:
# mn-cse-tenant-c
...
# If your restricted deployment is behind an auth gateway, pass EXTRA_CURL_ARGS.
...
create_ae() {
  ...
  local payload
  payload='{"m2m:ae":{"rn":"'"$ae_rn"'","api":"Nadn.'"$domain"'.v1","rr":true,"lbl":["domain:'"$domain"'","type:adn","node:'"$node_rn"'"],"poa":["http://localhost:28'"$idx"'0"]}}'
